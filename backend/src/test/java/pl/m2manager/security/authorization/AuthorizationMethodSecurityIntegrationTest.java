...
class AuthorizationMethodSecurityIntegrationTest {
	...
	@BeforeEach
	void setUp() {
		slug = "auth-it-" + UUID.randomUUID().toString().substring(0, 8);
		organization = saveOrganization("Auth IT Org", slug);
		userWithPermission = saveUser(organization, "authorized@example.com", "password");
		userWithoutPermission = saveUser(organization, "unauthorized@example.com", "password");

		Role role = saveRole(organization, "Viewer");
		assignPermission(role, "BUILDINGS_VIEW");
		assignRole(userWithPermission, role);
	}

	@Test
	void protectedEndpoint_withoutJwt_returns401() throws Exception {
		mockMvc.perform(get("/api/test/authorization/buildings-view"))
				.andExpect(status().isUnauthorized());
	}

	@Test
	void protectedEndpoint_authenticatedWithPermission_returns200() throws Exception {
		String token = loginAndExtractToken("authorized@example.com", "password");
	...
	@Test
	void protectedEndpoint_authenticatedWithoutPermission_returns403() throws Exception {
		String token = loginAndExtractToken("unauthorized@example.com", "password");
